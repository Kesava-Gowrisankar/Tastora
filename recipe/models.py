...
class Recipe(TimeStampedBase):
    ...
    prep_time = models.PositiveIntegerField(
        help_text="Time required to prepare ingredients in minutes"
    )
    total_time = models.PositiveIntegerField(
        help_text="Preparation time + Cooking Time in minutes",
    ...
    def difficulty_display(self):
        return dict(self.DifficultyLevels.choices).get(self.difficulty, 'Unknown')
    ...
    def clean(self):
        if self.prep_time and self.total_time and self.total_time < self.prep_time:
...
# Collection
# -----------------------------
class Collection(TimeStampedBase):
    title = models.CharField(max_length=200, db_index=True)
...
class RecipeImage(TimeStampedBase):
    recipe = models.ForeignKey(
        Recipe,
        on_delete=models.CASCADE,
        related_name='images'
